...
$ rosrun tesr_ros_cr3_pkg manipulation_pipeline.py
...
class GetObjectName(smach.State):
    def __init__(self):
        smach.State.__init__(self, outcome = ['send_value'],
                             input_keys = ['objectname_input'],
                             output_keys = ['objectname_output'])
    def execute(self, userdata):
        userdata.objectname_input = 1;
        userdata.objectname_output = "Waterbottle"
        rospy.loginfo('Executing state GETOBJECTNAME')
        rospy.loginfo('Executing the object name %s'%userdata.objectname_output)
        return 'send_value'
class GetObjectPose(smach.State):
    def __init__(self, userdata):
        ...
        self.object_name = userdata.objectpose_input
        self.x_pixel = None
        self.y_pixel = None
    ...
    def execute(self, userdata):
        ...
        userdata.objectpose_output = object_pose
...
class Pick(smach.State):
    def __init__(self):
        smach.State.__init__(self, outcomes = ['planning_completed', 'planning_failed'],
                             input_keys = ['pick_input'],
                             output_keys = ['pick_output'])
    def execute(self, userdata):
        rospy.init_node('manipulation_pipeline')
        ################
        ### pregrasp ###
        ################
        object_pose = userdata.pick_input
        print("pick service")
        # success => true false
        success = pick_service(object_pose, 'front')
        if success == True :
            userdata.pick_output = True
            return 'planning_completed'
        else:
            userdata.pick_output = False
            return 'planning_failed'

def main():
    rospy.init_node('smach_pick_state_machine')

    sm = smach.StateMachine(outcomes = ['planning_completed', 'planning_failed'])
    sm.userdata.sm_counter = 0
    with sm:
        smach.StateMachine.add('GET_OBJECT_NAME', GetObjectName(),
                               transitions = {'send_value':'GET_OBJECT_NAME'},
                               remapping = {'objectname_input':'sm_counter',
                                            'objectname_output':'objectname_transition'})
        smach.StateMachine.add('GET_OBJECT_POSE', GetObjectPose(),
                               transitions = {'detected':'PICK'},
                               remapping = {'objectpose_input':'objectname_transition',
                                            'objectpose_output':'objectpose_transition'})
        smach.StateMachine.add('PICK', Pick(),
                               transitions = {'planning_completed':'SUCCESS',
                                              'planning_failed':'ABORTED'},
                               remapping = {'pick_input':'objectpose_transition',
                                            'pick_output':'pick_transition'})
